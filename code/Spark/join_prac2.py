...
orders.join(order_items,on=orders.order_id== order_items.order_item_order_id ,how="right"). \
    groupby('order_item_order_id').sum('order_item_subtotal').orderBy(col('sum(order_item_subtotal)').desc()).show()